...
import static com.samsung.nnlp.models.neuronet.AFunction.*;
...
public class ActivationFunction implements Serializable {
    ...
    static public double GetDerivative(AFunction type, double weightedSum) {
        ...
        switch (type) {
            ...
            case SiLU: return GetFunction (type, weightedSum)+ GetFunction(Sigmoid, weightedSum) * (1 - GetFunction (type, weightedSum));

            case ELU:
                if (weightedSum < 0) return  Math.exp(weightedSum);
                else return 1;

            case SoftPlus: return GetFunction (Sigmoid, weightedSum);
